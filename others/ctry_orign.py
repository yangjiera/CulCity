...
def print_stats(loc_city, name_city, table):
    if table=='twitter':
        qry_base = "select distinct(U.id), U.g_home_country from twitter_tweets as T, twitter_profiles as U " + \
                "where ((`latitude` between " + str(loc_city[0]) + " and " + str(loc_city[1]) + ") and (`longitude` between " + str(loc_city[2]) + " and " + str(loc_city[3]) + ")) and "+\
                "T.userId=U.id"
    if table=='instgram':
        qry_base = "select distinct(U.id), U.g_home_country from inst_posts as T, inst_profiles as U " + \
                "where ((`latitude` between " + str(loc_city[0]) + " and " + str(loc_city[1]) + ") and (`longitude` between " + str(loc_city[2]) + " and " + str(loc_city[3]) + ")) and "+\
                "T.userId=U.id"
    ...
    print result_tier
    plt.savefig('ctry_orign/'+name_city+'_'+table)'''
    cur.execute(qry_base)
    data = cur.fetchall()
    #print data
...
if __name__ == '__main__':
    ...
    for name_city in name_cities:
        ...
        for table in tables:        
            print_stats(loc_city, name_city, table)
            #plotIndex += 1
