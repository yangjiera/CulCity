...
class QuadTree:
    ...
    def __init__(self, data, Idx, mins, maxs, depth=-1):
        ...
        if depth > 0:
            ...
            self.children.append(QuadTree(data_q1, Idx_q1, 
                                              [xmin, ymin], [xmid, ymid], depth - 1))

            self.children.append(QuadTree(data_q2, Idx_q2, 
                                              [xmin, ymid], [xmid, ymax], depth - 1))

            self.children.append(QuadTree(data_q3, Idx_q3, 
                                              [xmid, ymin], [xmax, ymid], depth - 1))

            self.children.append(QuadTree(data_q4, Idx_q4, 
                                              [xmid, ymid], [xmax, ymax], depth - 1))

        else:
            leaves_data.append(data)
            leaves_idx.append(Idx)
            quads.append([xmin, ymin, xmax-xmin, ymax-ymin])
            #print leaves_idx
