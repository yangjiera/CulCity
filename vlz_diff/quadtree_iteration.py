...
class QuadTree:
    ...
    def __init__(self, data, Idx, mins, maxs, leafsize, depth=-1):
        ...
        depth = 0
        queue = [[data, xmin, ymin, xmax, ymax, Idx]]
        while len(queue)!=0:
            depth += 1
            print depth
            queue_cpy = copy.copy(queue)
            for elem in queue_cpy:
                qdata = elem[0]
                xmin = elem[1]
                ymin = elem[2]
                xmax = elem[3]
                ymax = elem[4]
                mids = 0.5 * (np.asarray([xmin, ymin]) + np.asarray([xmax, ymax]))
                xmid, ymid = mids
                Idx = elem[5]
                if len(qdata) <= leafsize:
                    queue.remove(elem)
                # split the data into four quadrants
                #print data[0:10]
                index1 = (qdata[:, 0] < mids[0]) & (qdata[:, 1] < mids[1])
                data_q1 = qdata[index1]
                Idx_q1 = [Idx[i] for i in xrange(len(index1)) if index1[i]]

                index2 = (qdata[:, 0] < mids[0]) & (qdata[:, 1] >= mids[1])
                data_q2 = qdata[index2]
                Idx_q2 = [Idx[i] for i in xrange(len(index2)) if index2[i]]

                index3 = (qdata[:, 0] >= mids[0]) & (qdata[:, 1] < mids[1])
                data_q3 = qdata[index3]
                Idx_q3 = [Idx[i] for i in xrange(len(index3)) if index3[i]]

                index4 = (qdata[:, 0] >= mids[0]) & (qdata[:, 1] >= mids[1])
                data_q4 = qdata[index4]
                ...
                if len(data_q1) > leafsize:
                    queue.append([data_q1, xmin, ymin, xmid, ymid, Idx_q1])
                else:
                    if len(data_q1)>0:
                ...
                if len(data_q2) > leafsize:
                    queue.append([data_q2, xmin, ymid, xmid, ymax, Idx_q2])
                else:
                    if len(data_q2)>0:
                ...
                if len(data_q3) > leafsize:
                    queue.append([data_q3, xmid, ymin, xmax, ymid, Idx_q3])
                else:
                    if len(data_q3)>0:
                ...
                if len(data_q4) > leafsize:
                    queue.append([data_q4, xmid, ymid, xmax, ymax, Idx_q4])
                else:
                    if len(data_q4)>0:
